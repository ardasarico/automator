...
/** The signed-in user's usage counts for the Settings dialog, forwarded with their token. */
export async function GET(req: Request) {
  ...
  try {
    const result = await request(process.env.API_URL, accountUsageContract, { token });
