...
export async function GET(req: Request) {
  ...
  try {
    const result = await request(process.env.API_URL, listNodePresetsContract, { token });
...
export async function POST(req: Request) {
  ...
  // Refused here before the round trip; the API applies the same check and redacts the secrets.
  if (!isNodePresetInput(body))
    return authErrorResponse(req, new AuthApiError(422, "invalid_request"));
  try {
    const result = await request(process.env.API_URL, createNodePresetContract, {
      token,
      body,
      timeoutMs: 15_000,
    });
