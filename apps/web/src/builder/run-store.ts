...
export type RunState = {
  ...
  error: string | null;
  start(): void;
  finish(run: FlowRun): void;
  fail(error: string): void;
  reset(): void;
  /** Shows a stored run as if it had just finished, for the history's links into the canvas. */
  loadRun(run: FlowRun): void;
};

export function createRunStore(initialRun: FlowRun | null = null): StoreApi<RunState> {
  return createStore<RunState>((set) => ({
    status: initialRun?.status ?? "idle",
    run: initialRun,
    error: null,
    start: () => set({ status: "running", run: null, error: null }),
    finish: (run) => set({ status: run.status, run, error: null }),
    fail: (error) => set({ status: "failed", run: null, error }),
    reset: () => set({ status: "idle", run: null, error: null }),
    loadRun: (run) => set({ status: run.status, run, error: null }),
  }));
}
