  ...
  return (
    ...
    >
      <CheckboxPrimitive.Indicator
        className="absolute -inset-px flex items-center justify-center rounded-xs text-primary-foreground data-unchecked:hidden data-checked:bg-primary data-indeterminate:text-foreground"
        data-slot="checkbox-indicator"
        render={(props: React.ComponentProps<"span">, state: CheckboxPrimitive.Indicator.State) => (
          <span {...props}>
            {state.indeterminate ? (
              <RiSubtractLine aria-hidden="true" className="size-3.5 sm:size-3" />
            ) : (
              <RiCheckLine aria-hidden="true" className="size-3.5 sm:size-3" />
            )}
          </span>
        )}
      />
