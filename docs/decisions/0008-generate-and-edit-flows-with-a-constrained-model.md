...
- One seam, `LanguageModel` in `packages/flow-engine`, describes a chat request (messages, optional tools, response format) and its answer (text, tool calls). The API implements it over OpenAI-compatible chat completions with plain `fetch`; tests script it turn by turn. Without a key the API passes no model, AI nodes fail as unconfigured, and the AI routes answer 503. (Written when OpenRouter was the only provider: `withFallbackModel` later put OpenAI first, `OPENAI_API_KEY` / `OPENAI_MODEL`, with OpenRouter, `OPENROUTER_API_KEY` / `OPENROUTER_MODEL`, catching any model failure. The seam did not change.)
