...
- Everything that touches the outside world is injected through `RunOptions` and handed to executors on their context: `fetch`, `sleep`, `now`, the `LanguageModel`, the `ChainProvider`, the `DataProvider` (the owner's data tables), the `GraphGateway` (The Graph's subgraph gateway), the `SecretsResolver`, the `Sandbox` (QuickJS in WebAssembly in the API, absent in the browser), and an `AbortSignal` that cancels the run at the next node and aborts the node in flight. Tests script these seams; the browser omits what it cannot provide, and the affected nodes fail with a message that says so.
